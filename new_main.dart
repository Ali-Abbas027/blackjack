...
class TableArcPainter5 extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    // Draw seat rectangles (betting boxes)
    ...
    for (final pos in seatPositions) {
      canvas.drawRect(
        Rect.fromCenter(center: pos, width: 60, height: 80),
        seatPaint,
      );
    }
    // Dealer
    canvas.drawRect(
      Rect.fromCenter(center: dealerPos, width: 60, height: 80),
      seatPaint,
    );
  }
...
class _BlackjackGameState extends State<BlackjackGame> {
    ...
    if (isActive) {
      return Row(
        children: <Widget>[
          ElevatedButton(
            onPressed: () {
              _playerHit(hand, PlayerTurn.player1);
            },
          ...
          ElevatedButton(
            onPressed: () {
              _playerStand(PlayerTurn.player1);
            },
            style: ElevatedButton.styleFrom(
  ...
  Widget handValueDisplay(List<CardModel> hand) {
    return Text(
                      ...
                      child: Padding(
                        ...
                        child: Column(
                          ...
                          children: [
                            ...
                            Row(
                              children: player1
                                  .map((card) => cardWidget(card))
                                  .toList(),
                            ),
                            handValueDisplay(player1),
                            const SizedBox(height: 8),
                                  ...
                                  children: dealer
                                      .map(
                                        (card) => cardWidget(
                                          ...
                                          hidden:
                                              !dealerReveal &&
                                              dealer.indexOf(card) == 0,
                                        ),
                                      )
                                      .toList(),
                                ),
                                handValueDisplay(dealer),
                              ],
                            ),
                          ),
                          Positioned(
                            bottom: 40,
                            left: MediaQuery.of(context).size.width * 0.25 - 70,
                            child: Column(
                              children: [
                                Text(
                                  'Computer',
...
void main() {
  runApp(const MaterialApp(home: BlackjackGame()));
}
